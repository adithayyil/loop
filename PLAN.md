...
| (next day) | **DEMO APP REMOVED + DESIGN SYSTEM v2 ("signal").** Deleted the self-hosted invoice demo (`app/demo/**`, `lib/demo-data.ts`, invoice fixture, `LOOP_DEMO_URL`, `npm run tunnel`) — any public site is the target now; the agent defaults to `about:blank`. Then a full UI/UX overhaul off research into 2025–26 agent-tool patterns (ambient dark surfaces, editorial serif accents, mono data labels, single accent discipline, glass composer islands, faux browser chrome around live viewers, orbit status indicators). New design system: Space Grotesk UI + Instrument Serif display + JetBrains Mono data (next/font, self-hosted); ink `#0a0a0c` + signal-lime `#c9f24e` accent (red reserved for REC); film-grain overlay, ambient radial glows, light-beam cards, hover-reveal step actions, click-outside/Esc param menus, recording + run elapsed timers, file-card downloads, card-based library. All `lc-*`/`dk-*` classes replaced by one `lp-*` system across chat/review/run/library. Verified: `tsc` clean, 5/5 tests, `next build` green, all four surfaces render server-side with the new classes + fonts. |
